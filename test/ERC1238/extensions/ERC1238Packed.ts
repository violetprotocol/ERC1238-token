  ...
  describe("internal functions", () => {
    ...
    describe("Minting", () => {
      ...
      describe("Base Id for FTs", () => {
        ...
        it("should aggregate balances of different FTs with the same base id", async () => {
          ...
          await erc1238Packed.mint(tokenRecipient.address, tokenId_1, FT_AMOUNT, []);

          expect(await erc1238Packed.balanceFromBaseId(tokenRecipient.address, baseId)).to.eq(FT_AMOUNT * 2);
          expect(await erc1238Packed.balanceOf(tokenRecipient.address, tokenId_0)).to.eq(FT_AMOUNT);
          expect(await erc1238Packed.balanceOf(tokenRecipient.address, tokenId_1)).to.eq(FT_AMOUNT);
        });
      });
    ...
    describe("_burn", () => {
      ...
      it("should revert when burning a non-existent token id", async () => {
        await expect(erc1238Packed.connect(admin).burn(tokenRecipient.address, tokenId, burnAmount)).to.be.revertedWith(
    ...
    describe("_burnBatch", () => {
      ...
      it("should revert if the length of inputs do not match", async () => {
        await expect(
          erc1238Packed.connect(admin).burnBatch(tokenBatchRecipient.address, tokenBatchIds.slice(1), burnBatchAmounts),
        ).to.be.revertedWith("ERC1238: ids and amounts length mismatch");

        await expect(
          erc1238Packed.connect(admin).burnBatch(tokenBatchRecipient.address, tokenBatchIds, burnBatchAmounts.slice(1)),
        ).to.be.revertedWith("ERC1238: ids and amounts length mismatch");
      });
      ...
      it("should revert when burning a non-existent token id", async () => {
        ...
        await expect(
          erc1238Packed.connect(admin).burnBatch(tokenRecipient.address, tokenBatchIds, burnBatchAmounts),
        ).to.be.revertedWith("ERC1238: burn amount exceeds balance");
      });

      it("should properly burn tokens", async () => {
        await erc1238Packed.connect(admin).mintBatch(tokenRecipient.address, tokenBatchIds, mintBatchAmounts, data);

        await erc1238Packed.connect(admin).burnBatch(tokenRecipient.address, tokenBatchIds, burnBatchAmounts);

        tokenBatchIds.forEach(async (tokenId, i) =>
          expect(await erc1238Packed.balanceOf(tokenRecipient.address, tokenId)).to.eq(
      ...
      it("should emit a BurnBatch event", async () => {
        await erc1238Packed.mintBatch(tokenRecipient.address, tokenBatchIds, mintBatchAmounts, data);

        await expect(erc1238Packed.burnBatch(tokenRecipient.address, tokenBatchIds, burnBatchAmounts))
